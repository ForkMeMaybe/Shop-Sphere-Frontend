...
const Home = () => {
  ...
  if (!cartContext) {
    return (
      <h2 className="text-center text-red-500 text-xl mt-10">
        Error: CartContext is not available.
      </h2>
    );
  }
      ...
      <div className="flex items-center justify-center min-h-screen w-screen">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-semibold text-white-800">
            Loading products...
            <br />
            It might take some time to wake the free backend server. So hang in
            there. :)
          </h2>
        </div>
      </div>
  ...
  if (error)
    return (
      <div className="flex items-center justify-center min-h-screen">
        <h2 className="text-center text-red-500 text-xl mt-10 bg-red-50 px-6 py-4 rounded-lg shadow">
          {error.message}
        </h2>
      </div>
    );
  ...
  const handleAddToCart = async (product) => {
    ...
    try {
      ...
      setTimeout(() => setLoadingProductId(null), 700);
    }
  };
