    ...
    <div className="container mx-auto px-4 py-10">
      ...
      {orders.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-6 justify-center">
          {orders.map((order, orderIndex) => (
            <div key={orderIndex} className="p-4 border border-gray-300 rounded-lg shadow-md w-80">
              <p className="text-gray-500">Placed At: {new Date(order.placed_at).toLocaleString()}</p>
              <p className={`text-sm font-semibold ${order.payment_status === "Pending" ? "text-yellow-500" : "text-green-600"}`}>
                Status: {order.payment_status}
              </p>
              ...
              <div className="mt-4">
                ...
                <div className="flex flex-col gap-3">
                  {order.items.map((item, itemIndex) => (
                    <div key={itemIndex} className="flex items-center gap-4 border-b py-3">
                      <img
                        src={item.product.images[0]?.image_data || "/default-product.png"}
                        alt={item.product.title}
                        className="w-20 h-20 object-cover rounded-md border"
