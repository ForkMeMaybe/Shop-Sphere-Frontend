...
const Cart = () => {
  const { cartItems, totalPrice, removeFromCart, fetchCartData } = useContext(CartContext);
  const cartId = localStorage.getItem("cartId");
  const navigate = useNavigate();
  const [customer, setCustomer] = useState(null);
  const [addresses, setAddresses] = useState([]);
  const [loading, setLoading] = useState(true);
  const [operationLoading, setOperationLoading] = useState(null); // new state for item operations
                    ...
                    <tr key={item.id} className="group hover:bg-blue-50 transition-colors duration-200">
                      ...
                      <td className="py-6 px-6 text-center">
                        <button
                          ...
                          className="inline-flex items-center px-4 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors duration-200 disabled:opacity-50"
                          disabled={operationLoading === item.id}
                        >
                          {operationLoading === item.id ? (
                            <>
